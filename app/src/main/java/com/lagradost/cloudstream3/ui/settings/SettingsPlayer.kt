...
import com.lagradost.cloudstream3.ui.player.PlayerEngine
...
class SettingsPlayer : BasePreferenceFragmentCompat() {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        // WaveStream: in-app playback engine selector (ExoPlayer / MPV / VLC)
        getPref(R.string.player_engine_key)?.setOnPreferenceClickListener {
            val engineNames = mutableListOf(
                getString(R.string.player_engine_exo),
                getString(R.string.player_engine_vlc),
            )
            val engineValues = mutableListOf(
                PlayerEngine.ENGINE_EXO,
                PlayerEngine.ENGINE_VLC,
            )
            // libmpv needs Android 8.0+; hide it on older devices
            if (PlayerEngine.isMpvSupported) {
                engineNames.add(1, getString(R.string.player_engine_mpv))
                engineValues.add(1, PlayerEngine.ENGINE_MPV)
            }
            val current = settingsManager.getString(
                PlayerEngine.KEY, PlayerEngine.ENGINE_EXO
            ) ?: PlayerEngine.ENGINE_EXO

            activity?.showBottomDialog(
                engineNames,
                engineValues.indexOf(current).coerceAtLeast(0),
                getString(R.string.player_engine_title),
                true,
                {},
            ) { index ->
                settingsManager.edit {
                    putString(PlayerEngine.KEY, engineValues[index])
                }
                showToast(
                    String.format(
                        getString(R.string.player_engine_set),
                        engineNames[index]
                    )
                )
            }
