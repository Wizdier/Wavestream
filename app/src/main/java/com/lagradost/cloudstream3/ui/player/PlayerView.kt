...
class PlayerView @JvmOverloads constructor(
    ...
    var player: IPlayer = PlayerEngine.createPlayer()
    ...
    fun initialize() {
        ...
        // WaveStream: native engines (MPV/VLC) render into their own TextureView which we
        // insert into the Media3 content frame, and drive the stock controller UI through
    ...
    private fun playerUpdated(player: Any?) {
        ...
        } else if (player is NativeMedia3Adapter) {
            // WaveStream: native MPV/VLC engine attached through the Media3 adapter.
            mMediaSession?.release()
            mMediaSession = try {
                ...
                Log.e(TAG, "MediaSession for native adapter failed", t)
                null
            }
            exoPlayerView?.player = player
            exoPlayerView?.performClick()
